...
public class PaymentController {
    ...
    public ResultVo create(@RequestBody Payment payment) {
        ...
        if (result > 0) {
            return ResultVoUtil.success(result, "插入数据库成功,serverPort: " + serverPort);
        }
        return ResultVoUtil.error(444, "插入数据库失败");
    }
    ...
    public ResultVo getPaymentById(@PathVariable("id") Long id) {
        ...
        if (payment != null) {
            return ResultVoUtil.success(payment, "查询成功,serverPort:  " + serverPort);
        } else {
            return ResultVoUtil.error(444, "没有对应记录,查询ID: " + id);
        }
    }
